...
class ItemDeliveryConfiguratorInvasion : ItemDeliveryConfigurator {
	...
	// ----------------------------------------------------
	protected void setupUGVDetonator() {
		_log("adding ugv-to-detonator conversion config", 1);
		array<Resource@> deliveryList = {
			 Resource("ugv.projectile", "projectile")
		};

		array<Resource@> m_rewardItems = {
			Resource("detonator.weapon", "weapon")
		};   

		//GiftItemDeliveryRandomRewarder@ rewarder = GiftItemDeliveryRandomRewarder(m_metagame, rewardPasses);
		GiftItemDeliveryRewarder@ rewarder = GiftItemDeliveryRewarder(m_metagame, m_rewardItems);
